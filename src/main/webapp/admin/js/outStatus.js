...
$(document).ready(function(){
	...
	var data="page=1&payStatus=1&outStatus=0&takeStatus=0";
	$.ajax({
		"url":url,
		"data":data,
		"type":"POST",
...
function showOrderList(list){
	...
	for(var i=0;i<list.length;i++){
		var tr= '<tr><td><div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id="2"><i class="layui-icon">&#xe605;</i></div></td>'
    		...
    		+'<td>#{money}</td>'
    		+'<td>已支付</td>'
    		+'<td id="toOut#{id}">未发货</td>'
    		+'<td id="payWay#{id}"></td>'
    		+'<td>#{date}</td>'
    		+'<td class="td-manage">'
    		+'<a title="查看"  onclick="x_admin_show(\'查看\',\'userAddress.html?id=#{id}\')" href="javascript:;">'
			+'<i class="layui-icon">&#xe63c;</i></a>'
			+'<button class="layui-btn layui-btn-normal layui-btn-mini"  id="out#{id}" onclick="shipments(\'#{id}\')" style="display: inline-block;margin-left: 10px">发货</button>'
            +'<button class="layui-btn layui-btn-disabled layui-btn-mini"  id="outOk#{id}" style="display: none;margin-left: 10px">已发货</button>'
			+'</td></tr>';
...
function getOrderList(target){
	...
	if($(target).attr("class")=="first"){//首页
		data="page=1&payStatus=1&outStatus=0&takeStatus=0";
		$(".num").text("1");
		...
		page=nowPage-1;
		data="page="+page+"&payStatus=1&outStatus=0&takeStatus=0";
		$(".num").text(page);
...
function shipments(id){
	...
	var data="status=outStatus&statusNo=1&id="+id;
...
$("#search").click(function(){
	...
	if(data!=""){
		$.ajax({
			...
			"success":function(json){
				var list=json.data;
				if(list[0]==null){//1 0 0
					layer.msg("查无此单！",{icon:2,time:1500});
				}else if(list[0].payStatus==0){
					layer.msg("该单状态为未支付，请到待支付页查询详情！",{icon:2,time:2000});
				}else if(list[0].outStatus==1&&list[0].takeStatus==0){
					layer.msg("该单状态为未收货，请到待收货页查询详情！",{icon:2,time:2000});
				}else if(list[0].outStatus==1&&list[0].takeStatus==1){
