...
$(document).ready(function(){
	...
	var data="page=1&payStatus=1&outStatus=1&takeStatus=1";
	$.ajax({
		"url":url,
		"data":data,
		"type":"POST",
...
function getOrderList(target){
	...
	if($(target).attr("class")=="first"){//首页
		data="page=1&payStatus=1&outStatus=1&takeStatus=1";
		$(".num").text("1");
		...
		page=nowPage-1;
		data="page="+page+"&payStatus=1&outStatus=1&takeStatus=1";
		$(".num").text(page);
	...
	if(data!=""){
		$.ajax({
			...
			"success":function(json){
				var list=json.data;
				if(list[0]==null){
					layer.msg("查无此单！",{icon:2,time:2000});
				}else if(list[0].payStatus+list[0].outStatus+list[0].takeStatus!=3){
					layer.msg("该单状态为未完成！",{icon:2,time:2000});
				}else{
					showOrderList(list);
				}
			}
